var skill_type = {
  ...
  'cost':function(env,idx,data){
    var per = (data.per!==undefined) ? data.per : 0;
    var atk = (data.atk_r!==undefined) ? (data.atk_r * env.busho[idx].cost) : 0;
    var def = (data.def_r!==undefined) ? (data.def_r * env.busho[idx].cost) : 0;
    var des = (data.des_r!==undefined) ? (data.des_r * env.busho[idx].cost) : 0;
    var spd = (data.spd!==undefined) ? data.spd : 0;
    var descs = [];
    if(per != 0) descs.push("確率: "+per.toFixed(1)+"％");
    if(atk != 0) descs.push("攻: "+atk.toFixed(1)+"％（内訳: コスト×"+data.atk_r.toFixed(1)+" ％）");
    if(def != 0) descs.push("防: "+def.toFixed(1)+"％（内訳: コスト×"+data.def_r.toFixed(1)+" ％）");
    if(des != 0) descs.push("破壊: "+des.toFixed(1)+"％（内訳: コスト×"+data.des_r.toFixed(1)+" ％）");
    if(spd != 0) descs.push("速度: "+spd.toFixed(1)+"％");
